...
fun main(args: Array<String>) {
    ...
    val tracking = Config.getTrackingState()?.toBoolean() == true
    ...
    app.apply {
        ...
        get("/user/:user") { ctx ->
            val user = ctx.param("user")!!
            when (UserCtrl.hasStarredRepo(user) || unrestricted) {
                true -> ctx.renderVelocity("user.vm", model("user", user, "tracking", tracking))
        ...
        get("/search") { ctx ->
            ...
            when (UserCtrl.hasStarredRepo(user) || (unrestricted && user != "")) {
                ...
                false -> ctx.renderVelocity("search.vm", model("q", escapeHtml(user), "tracking", tracking))
            }
        }
